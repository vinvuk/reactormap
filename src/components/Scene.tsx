...
interface SceneProps {
  ...
  onLoadComplete: () => void;
  isLoading?: boolean;
  onHoverReactor?: (reactor: { name: string; status: string } | null, screenPos: { x: number; y: number } | null) => void;
...
function DynamicRotateSpeed({ controlsRef }: { controlsRef: React.RefObject<any> }) {
  ...
  useFrame(() => {
    ...
    // At max distance (~22 units from surface): rotateSpeed = 0.4
    // At min distance (~1.5 units from surface): rotateSpeed = 0.015
 ...
 * Combines Earth, reactor markers, camera, and lighting
